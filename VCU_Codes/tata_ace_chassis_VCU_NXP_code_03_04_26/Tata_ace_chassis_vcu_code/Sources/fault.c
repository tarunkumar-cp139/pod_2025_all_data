...
bool fault_can_mesg(void){//to indicate the fault sends to OBC
	//if(fault_checks_enable==1){
	         	fault.tx_data[0] =radar.can_fault<<4 | obc.can_fault<<4 | bms.can_fault<<3 | brake.can_fault<<2 | pdu.can_fault <<1 | mcu.can_fault ;//if the ECU CAN communication failed for a perticular time period can_fault will trigger
		if(wake_up_success){
