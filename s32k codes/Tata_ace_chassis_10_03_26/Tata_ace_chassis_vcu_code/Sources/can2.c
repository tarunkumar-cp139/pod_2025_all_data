...
void can2_config(){
    ...
    can_config_rx_mb_for_extid_filter(8U, speed_ctrl);
	...
	FLEXCAN_DRV_Receive(INST_CANCOM2,8U,&rx_can2_init);
  	can2.init_success=1;
  }
void sending_data_to_obc(){
							...
							obc.tx_data[7]=applied_press;//obc.check_sum;
...
//																             	LPUART_DRV_SendData(INST_LPUART1,tx_buff,strlen(tx_buff));
 		...
 		case speed_ctrl: //EPS
 	   	 		{
 	   	 		set_rpm=rx_can2_init.data[0]+rx_can2_init.data[1]*256;
 	   	 				  memset(rx_can1_init.data, 0, sizeof(rx_can2_init.data));
