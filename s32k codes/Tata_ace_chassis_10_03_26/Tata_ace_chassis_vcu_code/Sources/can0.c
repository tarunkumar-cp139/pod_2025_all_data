...
//bool can0_check;
  uint8_t byte_5,byte_6,press_valid,count_inc=0,press_bar=0,press_applied=0;
  volatile uint16_t twos16_via_func, twos_comp_u16 ,dec_u16,ones_comp_u16;
  float friction_coefficient=0.35,radius_of_wheel=0.31,radius_of_brake=0.11,area_of_pistion=1.256*1e-3;//(1.256/1000)  // 1/1000=1e-3
  double pressure ,pressure_in_bars,prev_pressure=20;
  int vehicle_load=1000;
 flexcan_data_info_t can0_data_init,can0_data_init_std;//,accelation
...
//FLEXCAN_DRV_Receive(INST_CANCOM3,6U,&rx_can0_init);//if in-case extended id triggers the interrupt will trigger
...
//		LPUART_DRV_SendData(INST_LPUART1,tx_buff,strlen(tx_buff));
		if((applied_press>=press_bar-6)&&(applied_press<=press_bar)){
			press_bar=press_bar-10;
//			sprintf(tx_buff,"press_bar = %d\r\n",press_bar);
//			LPUART_DRV_SendData(INST_LPUART1,tx_buff,strlen(tx_buff));
		}
		return 0;
	}
	else if(applied_press>=1 && applied_press<=10){
		if((applied_press>=press_bar-6)&&(applied_press<=press_bar)){
					press_bar=press_bar-1;
...
uint8_t pressure_to_apply(){
	      vehicle_load=vehicle_load+pay_load;
		  pressure=(vehicle_load*mcu.speed)/(area_of_pistion*friction_coefficient);//*radius_of_wheel / *radius_of_brake
		  pressure_in_bars=pressure*1e-5;//
		//  prev_pressure=pressure_in_bars;
		  if(pressure_in_bars>=160)
			  pressure_in_bars=160;
	if(pressure_in_bars>100 && pressure_in_bars<=160)
	{
	// if(mcu.speed>=1){//to make sure that minimum pressure is applied at 0 rpm also

	  return(uint8_t)pressure_in_bars/4;
	 }
	if(pressure_in_bars>50 && pressure_in_bars<=100)
	{
	// if(mcu.speed>=1){//to make sure that minimum pressure is applied at 0 rpm also

	  return(uint8_t)pressure_in_bars/3;
	 }
	if(pressure_in_bars>30 && pressure_in_bars<=50)
	{
	// if(mcu.speed>=1){//to make sure that minimum pressure is applied at 0 rpm also

	  return(uint8_t)pressure_in_bars/2;
	 }
	 else
		 return 20;
}
void sending_data_to_brake(){
						...
						brake.count_to_send_data=count_inc<<4;///byte 1
						press_valid=16;//byte 3
						byte_5=(press_bar*2)>>4;
						byte_6=((press_bar*2)&0x0f)<<4;

						     dec_u16    = (brake.count_to_send_data+press_valid+byte_5+byte_6);                 // 240
					   ...
					   twos_comp_u16    = (uint16_t)(ones_comp_u16 + 1u); // two's complement= one's complement+1 // Expected: ones_comp_u16 = 0xFF0F, twos_comp_u16 = 0xFF10
						...
						brake.tx_data[1]=brake.count_to_send_data;//count
						brake.tx_data[2]=0x0;
...
void can0_callback (uint8_t instance, flexcan_event_type_t eventType,uint32_t buffIdx, flexcan_state_t flexcanState)
 ...
 //(void)buffIdx;
   	...
   	case FLEXCAN_EVENT_RX_COMPLETE:
   	{
   		...
   		//FLEXCAN_DRV_Receive(INST_CANCOM3,6U,&rx_can0_init);//extended
   		FLEXCAN_DRV_Receive(INST_CANCOM3,8U,&rx_can0_init_std);
...
//   					brake.data[7]=rx_can0_init_std.data[7];
   	...
   	//   	 		byte5=brake.data[5];
   					...
   					memcpy(brake.data,rx_can0_init_std.data,8);
   					...
   					applied_press=brake.data[4]<<8|brake.data[5];
   	   	 		  	...
   	   	 		  	if((diagnosis_on==1) || (fault_checks_enable==1)){
   	   						if((brake.data[2]>>6 )& (0x03) == 1){
   	   							brake.fault_alarm=1;//fault yellow warning lamp
   	   						}
   	   						else if( (brake.data[2]>>6) &( 0x03 )== 0){
   	   							brake.fault_alarm=0;
   	   						}
   	   				    	else if((brake.data[2]>>6) & (0x03) == 2){
								brake.fault_alarm=1;//fault red warning lamp
							}
   	   				}
